...
const MyBills = () => {
  ...
  useEffect(() => {
    ...
    const userBills = slots.filter(s => s.booked && s.bookedUserEmail === loggedUser.email);

    const calculatedBills = userBills.map(slot => {
      const normalizedService = slot.serviceName?.trim().toLowerCase();
      const serviceCharge = slot.serviceCharge || SERVICE_PRICING[normalizedService] || 0;
  ...
  return (
    <>
      ...
      <div className="container py-4">
        <h3 className="mb-4">My Bills & Charges</h3>
        ...
        <table className="table table-bordered table-striped">
          ...
          <tbody>
            {bills.length === 0 ? (
              <tr><td colSpan="8" className="text-center">No bills available</td></tr>
            ) : (
              bills.map(bill => (
                <tr key={bill.id}>
                  <td>{bill.date}</td>
